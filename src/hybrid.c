...
int main(int argc, char *argv[]) {
    ...
    // Loop through thread counts from 1 to omp_threads
    for (int num_threads = 1; num_threads <= omp_threads; num_threads++) {
        ...
        #pragma omp parallel for reduction(+:local_phrase_count) num_threads(num_threads)
        ...
        if (rank == 0) {
            FILE *results_file = fopen("outputHybrid1.txt", "a");
            fprintf(results_file, "Processes: %d\tThreads: %d\tTime: %f\n", num_procs, num_threads, end_time - start_time);
            fclose(results_file);
        }
